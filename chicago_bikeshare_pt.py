...
with open(os.path.dirname(os.path.abspath(__file__)) + "\\chicago.csv", "r") as file_read:
...
def column_to_list(data: list, index: int):
    """
    Função para retornar uma coluna a partir de uma matriz
    Argumentos:
        data: matriz
        index: index da coluna
    Retorna:
        Retorne uma lista que representa os valores de uma coluna da matriz
        
    """
    column_list = [data[i][index] for i in range(0, len(data))]
...
def most_popular_gender(data_list: list):
    """
    Função que encontra o gênero mais popular em uma lista.
    Argumentos:
        data_list: Lista de gêneros
    Retorna:
        Uma string indicando o gênero mais popular "Male", "Female", ou "Equal" 
...
def count_user_types(user_type_list: list):
    """
    Função que conta os tipos de usuários presentes em data_list.
    Argumentos:
        user_type_list: Uma lista contendo os tipos de usuários a serem contabilizados.
    Retorna:
        A quantidade de ocorrências dos tipos fornecidos em data_list
...
quantity = count_user_types(types)
y_pos = list(range(len(types)))
plt.bar(y_pos, quantity)
plt.ylabel('Quantidade')
...
def median(data: list):
    """
    Função para calcular a mediana de uma lista já ordenada.
    Argumentos:
        data: Lista para cálcula da mediana.
    Retorna:
        mediana
...
def count_items(column_list: list):
    ...
    Retorna:
        Uma lista de valores [tipos[], contadores[]]
